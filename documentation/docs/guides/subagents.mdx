...
goose can autonomously decide to use subagents when it determines they would be beneficial for your task - you don't always need to explicitly request them. This happens automatically in autonomous [permission mode](/docs/guides/goose-permissions) (`auto`). Subagents are disabled in manual approval, smart approval, and chat-only modes.

**Note for this fork:** The default mode is `approve` (Manual Approval), which means automatic subagent creation is disabled out of the box. To enable it, set `GOOSE_MODE=auto` in your environment or configuration.
:::
